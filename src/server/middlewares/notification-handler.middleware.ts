...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'transaction_made.create': {
    roles: ['tenant-roles', 'tenant-owner-roles'],
    key: 'transaction_id',
    tenantPath: [],
    userPath: [],
  },
  'account_updated.update': { roles: ['tenant-roles'], key: 'account_id', tenantPath: [], userPath: [] },
...
const tenantRoles: string[] = ['account-owner', 'bank-administrator'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.bank.findFirst({

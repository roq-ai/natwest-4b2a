...
import { getUserProfiles } from 'apiSdk/user-profiles';
import { UserProfileInterface } from 'interfaces/user-profile';
import { getTransactions } from 'apiSdk/transactions';
import { TransactionInterface } from 'interfaces/transaction';

import { UserProfileListPage as ShowCaseList } from 'pages/user-profiles';
...
function DashboardPage() {
  ...
  const userProfileStat = useSWR<PaginatedInterface<UserProfileInterface>>(
    () => `/user-profiles`,
    () => getUserProfiles({ limit: 1, offset: 0 }),
  );

  const transactionStat = useSWR<PaginatedInterface<TransactionInterface>>(
    () => `/transactions`,
    () => getTransactions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'User Profiles',
      result: userProfileStat?.data?.totalCount,
      isLoading: userProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
      result: transactionStat?.data?.totalCount,
      isLoading: transactionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`user-profiles`} passHref legacyBehavior>
                  <Button

...
import { getBanks, deleteBankById } from 'apiSdk/banks';
import { BankInterface } from 'interfaces/bank';

type ColumnType = ColumnDef<BankInterface, unknown>;
...
export function BankListPage(props: BankListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BankInterface>>(
    () => `/banks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BankInterface) => {
    if (hasAccess('bank', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/banks/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/banks/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('bank', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/banks/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('bank', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();

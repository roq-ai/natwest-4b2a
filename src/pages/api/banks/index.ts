...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getBanksPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'bank');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.bank.count(countOptions as unknown),
  ...
  async function getBanks() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.bank
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'bank'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),

...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['account-owner', 'bank-administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BankInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/banks` : null),
    () => getBanks({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Banks',
      path: isTenantUser ? `/banks/view/${data?.[0]?.id}` : '/banks',
      entity: 'bank',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Accounts', path: '/accounts', entity: 'account', service: AccessServiceEnum.PROJECT, icon: FiBriefcase },
    {
      name: 'Transactions',
      path: '/transactions',
      entity: 'transaction',
